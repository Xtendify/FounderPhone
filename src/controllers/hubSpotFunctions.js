...
const REDIRECT_URL = process.env.SERVER_URL + '/hubspotcallback';
...
export async function getAccessToken(code) {
	...
	try {
		...
		return {
			success: false,
			error:
				'There was a problem connecting with HubSpot. Please try later',
		};
	}
}
	...
	try {
		...
		while (true) {
			...
			for (let contact of response.contacts) {
				...
				let identityProfile =
					contact['identity-profiles'][0].identities[0];
				if (identityProfile.type !== 'EMAIL')
					identityProfile =
						contact['identity-profiles'][0].identities[1];
