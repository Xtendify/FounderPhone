...
export async function reply(client, message, files, channel, isClientReply) {
...
export async function replyViaThread(
	...
	if (!messageId) {
		Slack.chat.postMessage({
			...
			token: client.slack.accessToken,
			text:
				'*From FounderPhone:* You have to respond to a thread to reply to a text. You can change your settings for this Slack at https://app.founderphone.com if you want each channel to be tied to a phone number instead. ' +
				CONTACT_MESSAGE,
		});
		return;
	}
