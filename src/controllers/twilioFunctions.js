...
export async function sendSMS(
	...
	await twilio.messages
		.create({
			...
			from: fromNumber,
			to: toNumber,
			statusCallBack: callbackId
				? `${serverUrl}/api/twilioStatusCallBack/${callbackId}`
				: null,
		})
		.catch((error) => {
			console.log(error);
...
export function sendSMSAsync(fromNumber, toNumber, message, callbackId = null) {
	let encodedMessage = getTextWithEmojisEncoded(message);
...
function savePhoneNumber(phoneNumber, clientEmail) {
	twilio.incomingPhoneNumbers
		.create({
			...
			smsUrl: `${process.env.SERVER_URL}/api/sms`,
			voiceUrl: `${process.env.SERVER_URL}/api/forwardcall`,
