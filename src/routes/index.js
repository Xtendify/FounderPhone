...
import { agenda, JOB_TYPES } from '../controllers/agenda';
...
const NOT_SUBSCRIBED_MESSAGE =
	"*From Founderphone*: You're not subscribed to FounderPhone. Please visit https://app.founderphone.com and subscribe to a plan to send a message";
...
router.post(
	...
	asyncHandler(async (req, res, next) => {
		...
		if (!client) {
			// to control excessive new phone numbers in Twilio
			// if (!process.env.NODE_ENV !== "production") {
			//   const clients = await Client.find();
			//   if (clients.length > 0) {
			//     return res.status(400).send(`Not allowed to create multiple accounts in non-production setup. Use either of ${clients.map(c => c.email).join(", ")} to login.`);
			//   }
			// }

			client = await createClient(firebaseUserId, {});
		}

		client.lastLoggedInOn = new Date();
		await client.save();
...
router.post(
	...
	asyncHandler(async (req, res, next) => {
		...
		if (response) {
			...
			return res
				.status(400)
				.send({ message: 'Error. ' + CONTACT_MESSAGE });
		}
	})
);
	...
	asyncHandler(async (req, res, next) => {
		...
		if (response) {
			if (response.success) {
				return res
					.status(200)
					.send(response.message ? response.message : '');
			} else {
				Sentry.captureException(error);
		...
		res.redirect(
			'https://slack.com/oauth/v2/authorize?' +
				querystring.stringify({
					...
					user_scope:
						'channels:read,files:read,files:write,groups:read',
			...
			if (accessTokenResponse.status === 200) {
				...
				if (
					...
					return res.status(200).send({
						...
						twilioPhoneNumber:
							slackAlreadyAssociated.twilioPhoneNumber,
					});
				} else {
					client.slack = {
						...
						userAccessToken:
							accessTokenResponse.data.authed_user.access_token,
...
router.post(
	...
	asyncHandler(async (req, res, next) => {
		...
		if (!client) {
			console.error(
				'Client not registered with this Slack team: ' + teamId
			);
			return;
		}
		...
		if (eventType === 'app_mention') {
			...
			if (client.slack.messageFormat === slackMessageFormat.CHANNEL) {
				reply(client, message, files, channel, true);
			} else {
				replyViaThread(
					client,
					message,
...
router.post(
	...
	asyncHandler(async (req, res, next) => {
		...
		if (client.slack) {
			...
			return res
				.status(400)
				.send('Tried to delete respondent without Slack');
		}
	})
);
...
router.post(
	...
	asyncHandler(async (req, res, next) => {
		...
		if (firstRespondentAdded) {
			const OUR_NUMBER = process.env.TWILIO_NUMBER;
			const MESSAGE =
...
router.post(
	...
	asyncHandler(async (req, res, next) => {
		...
		client.callRedirection.forwardToSlackFirst =
			req.body.forwardToSlackFirst;
	...
	asyncHandler(async (req, res, next) => {
		...
		if (!client) {
			Sentry.captureMessage(
				'Trying to SMS unassigned number: ' + twilioNumber
			);
		...
		if (!client.slack || !client.slack.accessToken) {
			Sentry.captureMessage(
				'Client is sending SMS but has not connected to Slack: ' +
					client.email
			);
			...
			return res.status(400).send({
				error: 'Client has not connected Slack to FounderPhone',
			});
		}
		...
		if (numberOfMedia > 0) {
			...
			for (
				...
				mediaAttachmentURLs.push(
					req.body[`MediaUrl${attachmentIndex}`]
				);
			}
...
router.get(
	...
	asyncHandler(async (req, res) => {
		...
		if (!client.slack || !client.slack.accessToken) {
			return res.status(404).send('Slack not connected');
		}
	...
	asyncHandler(async (req, res, next) => {
		...
		if (!client.callRedirection.forwardToSlackFirst) {
			...
			return res
				.status(200)
				.send(
					`<Response> <Dial> ${callRedirectNumber} </Dial> </Response>`
				);
		}
		...
		let incomingCallBlock = getIncomingCallBlock(
			callSid,
			formattedPhoneNumber
		);
	...
	asyncHandler(async (req, res, next) => {
		...
		if (!req.body.text) {
			return res
				.status(200)
				.send(
					`*From FounderPhone*: We need a phone number and message. Try using it like this: \`/founderphonetext ${process.env.TWILIO_NUMBER} Hi there!\` except with the number you want to text`
				);
		}
	...
	asyncHandler(async (req, res, next) => {
		...
		if (req.files.file.mimetype === 'text/csv') {
			...
			for (let contact of contacts) {
				let phoneNumbers = getPhoneNumbersFromCsvContact(
					contact,
					'phone'
				);
...
router.post(
	...
	asyncHandler(async (req, res, next) => {
		...
		if (!client)
			sendResponseToSlack(
				event.response_url,
				"Doesn't look like we're setup with this Slack. " +
					HELP_CENTER_MESSAGE
			);
		...
		if (event.type === 'block_actions') {
			...
			if (callSid) {
				let phoneNumber =
					event.view.state.values.phonenumberblock.forwarded_number
						.value;
				...
				if (!parsedPhoneNumber || !parsedPhoneNumber.isValid()) {
					mixpanel.track(
						'entered incorrect phone number for redirect',
						{
							distinct_id: client.email,
							clientPhone: client.twilioPhoneNumber,
						}
...
router.post(
	...
	asyncHandler(async (req, res) => {
		let scheduleAt = req.body.scheduleAt;

		if (!scheduleAt) {
			scheduleAt = moment().add(1, 'minutes');
		}

		const message = new CampaignList({
			client: res.locals.client._id,
			title: req.body.introMessage,
			status: 'SCHEDULED',
			phoneNumbers: req.body.contactsToSend,
		...
		const data = { campaignId: message._id };

		agenda.schedule(scheduleAt, JOB_TYPES.CAMPAIGN_SCHEDULE, data);

		return res.status(200).send({
			message:
				'Your message will be sent to ' +
				req.body.contactsToSend.length +
				' contacts',
		});
	})
);
...
router.post(
	...
	asyncHandler(async (req, res, next) => {
		...
		let contacts = await Contact.find(query, null, {
			sort: { name: 'asc' },
		});
...
router.post(
	...
	asyncHandler(async (req, res, next) => {
		await Contact.updateOne(
			{ _id: req.body.id, client: res.locals.client._id },
			{
				$set: {
...
router.post(
	...
	asyncHandler(async (req, res) => {
		...
		if (
			res.locals.client.billingDetails.currentPlan === planTypes.NOT_PAID
		) {
			sendMessageToSlack(
				channel,
				res.locals.client.slack.accessToken,
				NOT_SUBSCRIBED_MESSAGE
			);
			return;
		}
		...
		await reply(res.locals.client, message, files, channel, false);
		return res.status(200).send();
	})
);
...
router.post(
	...
	[validateFireBaseIdTokenMiddleware],
	asyncHandler(async (req, res, next) => {
		let savedTemplates = await MessageTemplate.find({
			client: res.locals.client._id,
		});
...
router.post(
	...
	asyncHandler(async (req, res, next) => {
		...
		let messageTemplate = await MessageTemplate.findById(
			req.body.templateId
		);
		if (!messageTemplate) {
...
router.get(
	'/twilioStatusCallBack/:messageId',
	[],
	asyncHandler(async (req, res, next) => {
		console.log('twilioStatusCallBack', req.body);
		...
		message.status = req.body.MessageStatus;
		await message.save();
